...
export default function EstandarCategoriasPage() {
  ...
  const handleAdd = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name) return;
    const newC = { id: `cat-${Date.now()}`, name, description: desc || 'Actividades y acceso', active: true, memberCount: 0 };
    setList([...list, newC]);
    setName('');
    setDesc('');
    toast({ type: 'success', title: 'Categoría Creada', message: `Categoría "${name}" añadida.` });
  };
              ...
              return (
                <div key={cat.id} className="p-4 flex items-center justify-between hover:bg-slate-50">
                  <div>
                    ...
                    <span className="text-[11px] text-polaco-700 font-semibold mt-1 inline-block">{count} socios activos</span>
                  </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs h-fit text-xs space-y-3">
          ...
          <form onSubmit={handleAdd} className="space-y-3">
            <div>
              ...
              <input type="text" required value={name} onChange={e => setName(e.target.value)} className="w-full p-2 border border-slate-300 rounded-lg" placeholder="Ej: Danzas Polacas" />
            </div>
            <div>
              <label className="block font-bold text-slate-700 mb-1">Descripción</label>
              <textarea rows={3} value={desc} onChange={e => setDesc(e.target.value)} className="w-full p-2 border border-slate-300 rounded-lg" placeholder="Descripción..." />
            </div>
            <button type="submit" className="w-full py-2 bg-polaco-600 hover:bg-polaco-700 text-white font-bold rounded-lg transition-colors">Guardar</button>
          </form>
        </div>
      </div>
    </div>
